📌 Project Overview

This Power BI project analyzes sales performance using transactional data to uncover trends in revenue, profitability, customer behavior, and product performance. The dashboard is designed to support data-driven business decisions by presenting key metrics and insights in a clear and interactive format.

🎯 Objectives

Monitor overall sales, profit, and order performance

Identify high-performing and underperforming products

Analyze profit margins across categories, regions, and discounts

Highlight products that require review or discontinuation

Enable stakeholders to quickly spot trends using KPIs and visuals

📈 Key KPIs

Total Sales

Total Profit

Profit Margin (%)

Order Count

Average Discount

All KPIs are dynamically calculated and driven by Order Date, enabling time-based analysis.

📊 Dashboard Visuals

KPI Cards for quick performance snapshot

Sales & Profit Trend Analysis by Order Date (Monthly/Yearly)

Category & Region-wise Profit Margin Matrix with conditional formatting

Discount vs Profit Margin Analysis to evaluate pricing strategy

Customer & Product Performance Analysis

Products to Review or Discontinue Table, identifying low-sales and loss-making products

🧠 Business Insights

Products with low sales and negative profit are flagged for review

Higher discounts generally correlate with lower profit margins

Technology category shows stronger margins compared to Furniture in certain regions

Regional performance varies significantly across product categories

🛠 Tools & Technologies

Power BI

DAX (Measures & Calculations)

SQL (Data extraction & transformation)

🚀 Outcome

This dashboard demonstrates practical business analytics skills, KPI design, and data storytelling using Power BI.

Data Modeling & Relationships

Conditional Formatting & KPI Indicators
